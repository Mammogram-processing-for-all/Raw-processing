...
전 단계가 모두 동원되는 가장 긴 파이프라인. 강도 선형화·경계 halo 방지·다중 스케일 강화·톤 매핑·DCM 패키징을 한데 묶은 상세 가이드는 [RAW → DCM 복원](raw-to-dcm.md). 각 단계의 효과는 [Quality Metrics](../quality-metrics/index.md)로 단계별 추적.
